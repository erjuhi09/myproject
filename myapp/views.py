...
def feedback(request,pk1,pk2):
    ...
    carts=Cart.objects.filter(user=user,status="active")
    request.session['carts']=len(carts)
    return render(request,'myapp/detail.html',{'user':user,'phone':phone,'flag':flag,'reviews':reviews})
...
def change_password(request):
	...
	UserObj=User.objects.get(pk=request.session['userpk'])
	if UserObj.password == oldpassword:
	#if UserObj:
		UserObj.password=cpassword
